...
class MIPGurobiSolver {
	...
	private void addXVariables() throws GRBException {
	...
	private void addWVariables() throws GRBException {
		wVar = new GRBVar[n];
		for (int i = 0; i < n; i++) {
	...
	private void addOneIncomingConstraint() throws GRBException {
		for (int v = 0; v < m + n; v++) {
	...
	private void addOneOutgoingConstraint() throws GRBException {
		for (int u = 0; u < m + n; u++) {
			GRBLinExpr expr = new GRBLinExpr();
	...
	private void addVehToReqLinConstraint() throws GRBException {
		for (int i = 0; i < n; i++) {
			GRBLinExpr expr = new GRBLinExpr();
			...
			for (int k = 0; k < m; k++) {
				VehicleData.Entry departure = vData.getEntry(k);
	...
	private void addReqToReqLinConstraint() throws GRBException {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				double t_ij = pathTravelTimeCalc.calcTravelTime(iReq.getToLink(), jReq.getFromLink());
	...
	private void applyInitialSolution(MIPSolution initialSolution) throws GRBException {
		for (int u = 0; u < m + n; u++) {
			for (int v = 0; v < m + n; v++) {
	...
	private MIPSolution extractSolution() throws GRBException {
